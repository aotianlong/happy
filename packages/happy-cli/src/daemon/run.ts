    ...
    const spawnSession = async (options: SpawnSessionOptions): Promise<SpawnSessionResult> => {
      ...
      try {
        ...
        if (useTmux && tmuxSessionName !== undefined) {
          ...
          // Determine agent command - support claude, codex, gemini, and cursor-agent
          const agent = options.agent === 'gemini' ? 'gemini' : (options.agent === 'codex' ? 'codex' : (options.agent === 'openclaw' ? 'openclaw' : (options.agent === 'cursor-agent' ? 'cursor-agent' : 'claude')));
        ...
        if (!useTmux) {
          ...
          // Construct arguments for the CLI - support claude, codex, gemini, and cursor-agent
